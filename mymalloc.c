...
//     heap_listp += (2*WSIZE); //블록 포인트를 여기 위치에 잡아주려고

//     if(extend_heap(CHUNKSIZE/WSIZE) == NULL){
//         return -1;
//     }

//     last_bp = heap_listp;   //next-fit을 위한 last_bp 초기화
//     return 0;
// }
...
//     size_t csize = GET_SIZE(HDRP(bp)); //bp로 부터 할당가능한 사이즈
...
//         last_bp = bp;
//         return bp;
//     }   //next-fit으로 처리 되면 bp 위치 처리하고 리턴

//     extendsize = MAX(asize, CHUNKSIZE);  // why max?
//     if ((bp = extend_heap(extendsize/WSIZE)) == NULL){   //할당할 수 있는 블록이라고 생각하면 되나? 없으면
//         return NULL;
//     }
//     place(bp, asize);
//     return bp;
